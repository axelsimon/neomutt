...
static int help_doclist_parse(struct Context *ctx)
{
  ...
  ctx->mailbox->msg_count = ctx->hdrmax = DocList->size;
  ...
  mutt_str_strfcpy(ctx->mailbox->realpath, HelpDocDir, sizeof(ctx->mailbox->realpath));
...
static int hdoc_open(struct Context *ctx, struct Message *msg, int msgno)
{
  ...
  char path[PATH_MAX];
  snprintf(path, sizeof(path), "%s/%s", ctx->mailbox->realpath, ctx->hdrs[msgno]->path);
...
int hbox_path_probe(const char *path, const struct stat *st)
{
  if (!path)
    return MUTT_UNKNOWN;
